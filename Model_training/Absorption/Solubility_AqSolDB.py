...
def check_muegge(mol):
    ...
    tpsa = Descriptors.TPSA(mol)
    return "Yes" if (
        200 <= mw <= 600 and -2 <= logp <= 5 and h_donors <= 5 and
        h_acceptors <= 10 and rot_bonds <= 15 and tpsa <= 150
    ) else "No"
...
def train_model(train_csv: str, smiles_col="Drug", label_col="Y", save_model_path=None):
    ...
    if save_model_path:
        joblib.dump(
            {"model": model, "smiles_col": smiles_col, "label_col": label_col},
            save_model_path
        )
...
def evaluate_model(model, test_csv, smiles_col="Drug", label_col="Y"):
    ...
    print("\n===== Solubility_AqSolDB Regression Metrics =====")
...
def predict_and_save(model, valid_csv, out_csv, smiles_col="Drug"):
    ...
    # Convert continuous prediction into normalized pseudo-class probability
    df["Prob_Class_0"] = 1 - df["Prediction"] / df["Prediction"].max()
    df["Prob_Class_1"] = df["Prediction"] / df["Prediction"].max()

    # Predicted class (rule-based)
    df["Predicted_Class"] = df[
        ["Lipinski", "Ghose", "Veber", "Egan", "Muegge"]
    ].apply(lambda row: "Yes" if list(row).count("Yes") >= 3 else "No", axis=1)

    # Column Order (kept consistent for UI)
    columns_order = [
        "Drug_ID","Drug","Y","MolWt","LogP","NumHDonors","NumHAcceptors",
